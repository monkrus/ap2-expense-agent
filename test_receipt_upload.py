...
def upload_receipt(token, org_id, expense_id, receipt_path):
    ...
    with open(receipt_path, 'rb') as f:
        ...
        response = requests.post(
            f"{BASE_URL}/receipts/upload/{expense_id}",
            headers=headers,
            files=files
        )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(SAMPLES_DIR):
        ...
        print("Run 'python generate_sample_receipts.py' first to create sample receipts.")
        exit(1)
    ...
    for i, expense_data in enumerate(SAMPLE_EXPENSES, 1):
        ...
        try:
            # Create expense
            print(f"   [{i}] Creating expense: {expense_data['vendor']} (${expense_data['amount']})")
