...
def main():
    ...
    initial_used = initial_stats.get('payment_mandates', {}).get('completed', 0) if initial_stats else 0
    ...
    result1 = submit_expense(token, org_id, {
        "amount": 45.00,
    ...
    result2 = submit_expense(token, org_id, {
        "amount": 35.00,
        "category": "MEALS",
    ...
    result3 = submit_expense(token, org_id, {
        "amount": 500.00,
        "category": "TRAVEL",
    ...
    final_used = final_stats.get('payment_mandates', {}).get('completed', 0) if final_stats else 0
    ...
    print(f"\n📊 Test Results:")
    print(f"  Tier 1 (AP2): {'✓ PASSED' if result1.get('auto_approved_via') == 'intent_mandate' else '✗ FAILED'}")
    print(f"  Tier 2 (Policy): {'✓ PASSED' if result2.get('auto_approved_via') == 'approval_policy' else '⚠ CHECK'}")
    print(f"  Tier 3 (Manual): {'✓ PASSED' if result3.get('status') == 'PENDING' else '✗ FAILED'}")
