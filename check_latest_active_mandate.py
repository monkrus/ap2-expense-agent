#!/usr/bin/env python3
"""
Check the latest ACTIVE mandate constraints
"""
...
def main():
    print("=" * 60)
    print("Checking Latest Active Mandate")
    print("=" * 60)
    ...
    try:
        # Get the most recent ACTIVE intent mandate
        cursor.execute("""
        ...
        if not result:
            print("\n❌ No active mandates found in database")
            return

        mandate_id, user_id, constraints_json, status, created_at = result

        print(f"\n📋 Most Recent Active Mandate:")
