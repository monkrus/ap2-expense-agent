...
if login_resp.status_code != 200:
    print(f"FATAL: Login failed with status {login_resp.status_code}: {login_resp.text}")
    sys.exit(1)
...
print("\n--- 2a: Intent mandate with full constraints ---")
resp = requests.post(f"{BASE_URL}/api/ap2/intent-mandate", json={
    "constraints": {
        "max_amount": 500.00,
...
if resp.status_code == 200:
    ...
    log_issue("BUG", "Failed to create intent mandate with minimal constraints", resp.text)
...
if resp.status_code == 200:
    ...
    log_issue("BUG", "Failed to create intent mandate with empty constraints", resp.text)
...
resp = requests.post(f"{BASE_URL}/api/ap2/complete-flow", json={
    "items": [
        {"id": "item-1", "description": "Laptop Stand", "amount": 45.99, "category": "office_supplies"},
        {"id": "item-2", "description": "USB Hub", "amount": 29.99, "category": "hardware"}
    ],
    "merchant": "Amazon",
    "constraints": {
        "max_amount": 200.00,
...
if resp.status_code == 200:
    ...
    # Now revoke it
    print("  Revoking...")
    revoke_resp = requests.post(f"{BASE_URL}/api/ap2/intent-mandate/{revoke_id}/revoke", json={
...
FRONTEND_STATUS_FILTERS = {"all", "active", "pending", "completed", "failed", "expired", "revoked"}
...
for status in statuses_found:
    if status not in FRONTEND_STATUS_FILTERS and status != "all":
        log_issue("MISMATCH", f"Backend status '{status}' has no frontend filter button",
...
for type_ in types_found:
    if type_ not in FRONTEND_TYPE_FILTERS and type_ != "all":
        log_issue("MISMATCH", f"Backend type '{type_}' has no frontend filter button",
...
resp_with_deleted = requests.get(f"{BASE_URL}/api/ap2/user/mandates?limit=50&include_deleted=true", headers=HEADERS)
if resp_with_deleted.status_code == 200:
    ...
    print(f"  Mandates with include_deleted=true: {len(with_deleted)} (deleted: {deleted_count})")

    if deleted_count > 0:
        # Frontend has no "deleted" filter button
        log_issue("MISMATCH", "Backend can return 'deleted' status but frontend has no filter for it",
...
for i, m in enumerate(all_mandates):
    ...
    for field in required_fields:
        if field not in m or m[field] is None:
            log_issue("BUG", f"Mandate missing required field '{field}'",
    ...
    if "id" in m and m["id"] is not None:
        if not isinstance(m["id"], str):
            log_issue("BUG", "Mandate id is not a string",
    ...
    if m.get("id") and len(m["id"]) < 12:
        log_issue("UX", "Mandate ID shorter than 12 characters",
    ...
    if m["type"] == "intent":
        ...
        if constraints is not None and not isinstance(constraints, dict):
            log_issue("BUG", "Intent mandate constraints is not a dict",
        ...
        if merchant is None and constraints:
            ...
            if constraints.get("merchant") or constraints.get("merchants"):
                log_issue("LOGIC", "Merchant exists in constraints but top-level merchant is None",
        ...
        if "total" not in m:
            log_issue("MISMATCH", "Cart mandate missing 'total' field",
                      "Frontend displays ${parseFloat(mandate.total).toFixed(2)} - will show NaN")
        if "merchant" not in m:
            log_issue("MISMATCH", "Cart mandate missing 'merchant' field",
                      "Frontend checks mandate.merchant for rendering")

    elif m["type"] == "payment":
        # Frontend accesses: m.payment_method
        if "payment_method" not in m:
            log_issue("MISMATCH", "Payment mandate missing 'payment_method' field",
                      "Frontend displays via {mandate.payment_method}")
        # Check: does payment mandate have total or merchant?
        if "total" not in m:
            log_ok("Payment mandate has no 'total' (expected - backend doesn't provide it)")
        if "merchant" not in m:
            log_ok("Payment mandate has no 'merchant' (expected - backend doesn't provide it)")
...
for m in intent_mandates:
    merchant = m.get("merchant")
    constraints_merchant = m.get("constraints", {}).get("merchant") if isinstance(m.get("constraints"), dict) else None
    if merchant is None and constraints_merchant:
        log_issue("LOGIC", "Search by merchant may miss intent mandates",
        ...
        log_ok(f"Intent mandate {m['id'][:20]}... has no merchant at all (correct for unconstrained)")
...
log_ok("Frontend uses .includes() not regex, so special chars like $, ^, (, ), [, ] won't crash")
...
for m in all_mandates:
    ...
    if m.get("constraints") is None:
        ...
        pass
log_ok("Null-safe checks: Frontend uses && short-circuit and ?. optional chaining correctly")
...
for type_filter in ["intent", "cart", "payment"]:
    resp = requests.get(f"{BASE_URL}/api/ap2/user/mandates?mandate_type={type_filter}&limit=50", headers=HEADERS)
    if resp.status_code == 200:
        filtered = resp.json().get("mandates", [])
        wrong_types = [m for m in filtered if m["type"] != type_filter]
        if wrong_types:
            log_issue("BUG", f"Type filter '{type_filter}' returned wrong types",
                      f"Got {len(wrong_types)} mandates with wrong type: {set(m['type'] for m in wrong_types)}")
        else:
            log_ok(f"Type filter '{type_filter}' returned {len(filtered)} mandates, all correct type")
    else:
        log_issue("BUG", f"Type filter '{type_filter}' API call failed", f"Status: {resp.status_code}")

# 7b. Filter by status
for status_filter in ["active", "pending", "completed", "failed", "expired", "revoked"]:
    resp = requests.get(f"{BASE_URL}/api/ap2/user/mandates?status_filter={status_filter}&limit=50", headers=HEADERS)
    if resp.status_code == 200:
        filtered = resp.json().get("mandates", [])
        wrong_statuses = [m for m in filtered if m["status"] != status_filter]
        if wrong_statuses:
            log_issue("BUG", f"Status filter '{status_filter}' returned wrong statuses",
                      f"Got {len(wrong_statuses)} mandates with wrong status: {set(m['status'] for m in wrong_statuses)}")
        else:
            log_ok(f"Status filter '{status_filter}' returned {len(filtered)} mandates, all correct status")
    else:
        log_issue("BUG", f"Status filter '{status_filter}' API call failed", f"Status: {resp.status_code}")

# 7c. Combined filter: type + status
print("\n--- Combined filters ---")
resp = requests.get(f"{BASE_URL}/api/ap2/user/mandates?mandate_type=intent&status_filter=active&limit=50", headers=HEADERS)
if resp.status_code == 200:
    filtered = resp.json().get("mandates", [])
    wrong = [m for m in filtered if m["type"] != "intent" or m["status"] != "active"]
    if wrong:
        log_issue("BUG", "Combined filter intent+active returned wrong results",
                  f"Got {len(wrong)} wrong mandates")
    else:
        log_ok(f"Combined filter intent+active returned {len(filtered)} correct mandates")

resp = requests.get(f"{BASE_URL}/api/ap2/user/mandates?mandate_type=intent&status_filter=expired&limit=50", headers=HEADERS)
if resp.status_code == 200:
    filtered = resp.json().get("mandates", [])
    log_ok(f"Combined filter intent+expired returned {len(filtered)} mandates")

resp = requests.get(f"{BASE_URL}/api/ap2/user/mandates?mandate_type=payment&status_filter=failed&limit=50", headers=HEADERS)
if resp.status_code == 200:
    filtered = resp.json().get("mandates", [])
        ...
        if isinstance(constraints, dict) and "merchants" in constraints:
            ...
            if isinstance(merchants_list, list) and len(merchants_list) > 0:
                ...
                if merchant != expected_merchant:
                    log_issue("BUG", "Merchant extraction from merchants list is incorrect",
        ...
        if isinstance(constraints, dict) and "categories" in constraints:
            ...
            if isinstance(categories_list, list) and len(categories_list) > 0:
                ...
                if category != expected_category:
                    log_issue("BUG", "Category extraction from categories list is incorrect",
...
for m in all_mandates:
    ...
    if merchant and ("," in merchant or '"' in merchant or "\n" in merchant):
        log_issue("BUG", "CSV export vulnerable to field injection",
...
log_issue("UX", "Export CSV missing important columns",
...
log_issue("BUG", "Export created_at parsing may fail in some browsers",
...
log_issue("LOGIC", "Export CSV not properly escaped",
...
if resp.status_code == 200:
    ...
    log_issue("LOGIC", "Stats prop is passed but never used by AgentActivityMonitor",
    ...
    log_issue("UX", "Stat cards don't count 'expired' and 'revoked' mandates",
...
for m in payment_mandates:
    ...
    if "total" not in m:
        log_issue("UX", "Payment mandates have no total amount displayed",
    ...
    if "merchant" not in m:
        log_issue("UX", "Payment mandates have no merchant displayed",
        ...
        if isinstance(constraints, dict):
            ...
            if has_merchant_key and not has_merchants_key:
                ...
                if actual_merchant != expected:
                    log_issue("BUG", "Merchant extraction fails when only 'merchant' key exists (no 'merchants')",
        ...
        if isinstance(constraints, dict):
            ...
            if has_category_key and not has_categories_key:
                ...
                if actual_category != expected:
                    log_issue("BUG", "Category extraction fails when only 'category' key exists (no 'categories')",
...
log_issue("BUG", "Revoke endpoint sets columns that don't exist on the model",
...
if revoke_id:
    resp = requests.get(f"{BASE_URL}/api/ap2/user/mandates?status_filter=revoked&limit=50", headers=HEADERS)
    if resp.status_code == 200:
        revoked = resp.json().get("mandates", [])
        for m in revoked:
            if m["id"] == revoke_id:
                # The API response for mandates doesn't include revoked_at anyway
                log_issue("UX", "Revoked mandates don't show revocation timestamp or reason in Activity tab",
...
log_issue("UX", "Timeline line extends below the last item",
...
for m in all_mandates:
    if m.get("created_at") is None:
        log_issue("BUG", "Mandate has null created_at",
        ...
        if "T" not in ca and " " not in ca:
            log_issue("BUG", f"Unusual created_at format: {ca}",
...
for m in all_mandates:
    if m["type"] == "intent":
        ...
        if exp is not None:
            ...
            log_issue("LOGIC", "Intent mandate has no expiration",
...
log_issue("LOGIC", "Frontend does client-side filtering, ignoring backend filter capabilities",
...
if resp.status_code == 200:
    ...
    print(f"  limit=2: returned {returned_count} mandates, count field says {stated_count}")
    if stated_count > returned_count:
        log_issue("LOGIC", "count field is larger than actual returned mandates",
        ...
        log_ok("count matches returned mandates count (user has <= 2 mandates or all types)")
...
if resp.status_code == 200:
    ...
    if total > 50:
        log_issue("BUG", "More than 'limit' mandates returned",
...
log_issue("MISMATCH", "Cart mandates can have 'approved' status but frontend has no config for it",
