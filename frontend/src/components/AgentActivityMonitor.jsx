...
import {
  ...
  TrendingUp, Target, DollarSign, Calendar, ChevronRight,
...
const AgentActivityMonitor = ({ mandates, stats }) => {
  ...
  const completedTransactions = mandates.filter(m => m.status === 'completed').length;
  const activeTransactions = mandates.filter(m => m.status === 'active' || m.status === 'pending').length;
  const failedTransactions = mandates.filter(m => m.status === 'failed').length;

  return (
    <div className="space-y-6">
      {/* Header & Stats */}
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Agent Activity</h2>
...
const ActivityTimelineItem = ({ mandate, isFirst }) => {
  ...
  return (
    <div className="px-6 py-4 hover:bg-gray-50 transition-colors relative">
      {/* Timeline Dot & Line */}
      <div className="absolute left-0 top-4 bottom-0 w-px bg-gray-200 ml-10" style={{ display: isFirst ? 'none' : 'block' }}></div>
      ...
      <div className="flex items-start space-x-4">
        ...
        <div className="relative flex-shrink-0">
          ...
          <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full ${statusConfig.dot} border-2 border-white`}></div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between mb-2">
            ...
            <div className="text-right flex-shrink-0 ml-4">
              <p className="text-xs text-gray-500">{formatDate(mandate.created_at)}</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-4 text-sm text-gray-600 mt-3">
            ...
            {mandate.merchant && (
              <div className="flex items-center space-x-1">
                <span className="font-medium text-gray-900">{mandate.merchant}</span>
              </div>
            )}

            {mandate.payment_method && (
              <div className="flex items-center space-x-1">
                <span className="text-gray-500">via {mandate.payment_method}</span>
              </div>
            )}
          </div>
          ...
          {mandate.type === 'intent' && mandate.expiration && (
            <div className="mt-2 text-xs text-gray-500">
              Expires: {new Date(mandate.expiration).toLocaleDateString('en-US', {
                month: 'short',
                day: 'numeric',
                year: 'numeric',
                hour: 'numeric',
...
const exportActivity = (mandates) => {
  const csv = [
    ['ID', 'Type', 'Status', 'Total', 'Merchant', 'Created At', 'Timestamp'].join(','),
    ...mandates.map(m => [
      m.id,
      m.type,
      m.status,
