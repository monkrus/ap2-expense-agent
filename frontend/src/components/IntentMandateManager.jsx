...
import {
  Plus, Target, Clock, CheckCircle, XCircle, AlertCircle,
  ChevronDown, ChevronUp, Edit2, Trash2, Eye, DollarSign,
  Calendar, Tag, Store, Shield
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Manage your AI agent's authorization to make purchases on your behalf
          </p>
        </div>
        <button
        ...
        {['all', 'active', 'expired', 'used'].map((filterOption) => (
          ...
          >
            ...
            <span className="ml-2 text-xs opacity-60">
              ({intentMandates.filter(m => filterOption === 'all' || m.status === filterOption).length})
            </span>
          </button>
        ))}
      </div>
      ...
      {filteredMandates.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-gray-900 mb-2">No Intent Mandates Yet</h3>
          <p className="text-gray-600 mb-6 max-w-md mx-auto">
            Create your first Intent Mandate to authorize your AI agent to handle expenses automatically.
          </p>
          <button
            onClick={onCreateMandate}
...
const MandateCard = ({ mandate, expanded, onToggle, onDelete, getStatusBadge }) => {
  ...
  let constraints = {};
  try {
    constraints = typeof mandate.constraints === 'string'
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {constraints.approval_required !== undefined && (
              <ConstraintItem
                ...
                value={constraints.approval_required ? 'Yes' : 'No (Auto-approve)'}
              />
            )}
            ...
            {/* Custom Constraints */}
            {Object.keys(constraints).filter(key =>
              !['max_amount', 'categories', 'merchants', 'approval_required', 'recurring'].includes(key)
            ).map((key) => (
              <ConstraintItem
                key={key}
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
              <div className="flex items-start space-x-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-gray-900">Cryptographic Signature</p>
