...
const OrganizationManagement = () => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const result = await organizationAPI.checkNameAvailability(
          createOrgForm.name,
        );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const result = await organizationAPI.checkSlugAvailability(
          createOrgForm.slug,
        );
    ...
    try {
      ...
      if (data.length > 0 && !currentOrg) {
        ...
        const savedOrg = savedOrgId
          ? data.find((org) => org.id === savedOrgId)
          : null;
  ...
  const handleBulkInvite = async () => {
    ...
    try {
      ...
      if (results.failed.length > 0) {
        // Show first error message if all failed
        if (results.successful.length === 0) {
          showError(results.failed[0].error || "Failed to send invitations");
        } else {
          // Show mixed results
          success(
            `Sent: ${results.successful.length} successful, ${results.failed.length} failed`,
          );
        }
      } else {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
                      ...
                      {members.map((member) => (
                        ...
                        >
                          ...
                          <div className="flex items-center gap-3">
                            <span
                              className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1 ${getRoleColor(member.role)}`}
                            >
                      ...
                      {invitations.length === 0 ? (
                        ...
                        invitations.map((inv) => (
                          ...
                          >
                            ...
                            <div className="flex items-center gap-3">
                              <span
                                className={`px-3 py-1 rounded-full text-xs font-medium ${getRoleColor(inv.role)}`}
                              >
                                {inv.role}
                              </span>
                    ...
                    <div className="bg-gray-50 rounded-lg p-6">
                      ...
                      <dl className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <dd className="text-base font-mono text-gray-700">
                            {currentOrg.slug}
                          </dd>
                        </div>
                        <div>
          ...
          <form onSubmit={handleCreateOrg} className="space-y-4">
            <div>
              ...
              {nameValidation.result && !nameValidation.result.available && (
                <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  {nameValidation.result.suggestions &&
                    nameValidation.result.suggestions.length > 0 && (
                      <div className="mt-1">
                        <p className="text-xs text-yellow-700 mb-1">
                          Try these instead:
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {nameValidation.result.suggestions.map(
                            (suggestion, idx) => (
                              <button
                                key={idx}
                                type="button"
            ...
            <div>
              ...
              {slugValidation.result && !slugValidation.result.available && (
                <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded-lg">
                  ...
                  {slugValidation.result.suggestions &&
                    slugValidation.result.suggestions.length > 0 && (
                      <div className="mt-1">
                        <p className="text-xs text-yellow-700 mb-1">
                          Try these instead:
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {slugValidation.result.suggestions.map(
                            (suggestion, idx) => (
                              <button
                                key={idx}
                                type="button"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  <strong>Why UTC?</strong> UTC ensures consistent timestamps
                  across global teams and prevents daylight saving confusion.
                  Recommended for international organizations.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditOrgForm({ ...editOrgForm, currency: e.target.value })
                  }
                  className="w-full px-3 py-2 border rounded-lg"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditOrgForm({ ...editOrgForm, timezone: e.target.value })
                  }
                  className="w-full px-3 py-2 border rounded-lg"
                >
                  <optgroup label="Universal">
      ...
      {/* Upgrade Prompt for Tier Limits */}
      <UpgradePrompt
        ...
        description={
          ...
          "Upgrade your plan to unlock additional features and grow your team."
        }
        recommendedPlan={
