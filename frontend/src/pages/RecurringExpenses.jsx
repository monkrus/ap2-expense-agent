...
const RecurringExpenses = () => {
  ...
  const handlePause = async (templateId) => {
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetch(
        `/api/recurring-expenses/${templateId}/pause`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleResume = async (templateId) => {
    try {
      const token = localStorage.getItem("access_token");
      const response = await fetch(
        `/api/recurring-expenses/${templateId}/resume`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Schedule automatic expense submissions for fixed, repeating
                costs like subscriptions and rent. Expenses are created on your
                behalf and sent for approval automatically.
              </p>
            </div>
            <button
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-semibold text-gray-900">
                            {template.total_submitted} times
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      <button
                        onClick={() =>
                          handleDelete(template.id, template.vendor)
                        }
