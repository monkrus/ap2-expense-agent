...
if resp.status_code == 200:
    d = resp.json()
    log("T7-empty-items", "FAIL", f"Backend accepted empty items array. ap2_flow_complete={d.get('ap2_flow_complete')}")
...
if resp.status_code == 200:
    ...
    log("T8-missing-amount", "FAIL", f"Unexpected: {resp.status_code}: {resp.text[:200]}")
...
if resp.status_code == 200:
    ...
    log("T10-constraint-violation", "FAIL", f"Unexpected: {resp.status_code}: {resp.text[:200]}")
...
print(f"Response: {resp.text[:500]}")
if resp.status_code == 200:
    log("T12-category-mismatch", "FAIL", "Backend accepted items with wrong category vs constraint")
elif resp.status_code in (400, 422):
...
print(f"Response: {resp.text[:500]}")
if resp.status_code == 200:
    log("T20-special-chars", "FAIL", "Accepted HTML/script tags in fields - stored XSS risk")
elif resp.status_code in (400, 422):
...
if resp.status_code == 200:
    log("T21-long-strings", "FAIL", "Accepted 5000-char merchant. DB column is VARCHAR(255)")
elif resp.status_code == 500:
    log("T21-long-strings", "FAIL", "500 error - DB column overflow, no input validation")
elif resp.status_code in (400, 422):
    log("T21-long-strings", "PASS", f"Rejected: {resp.status_code}")
else:
    log("T21-long-strings", "FAIL", f"Unexpected: {resp.status_code}: {resp.text[:200]}")
...
print(f"Response: {resp.text[:500]}")
if resp.status_code == 200:
    log("T23-mixed-categories", "FAIL", "Accepted mixed categories with single-category constraint")
elif resp.status_code in (400, 422):
...
if resp.status_code == 200:
    d = resp.json()
    log("T6-large-amount", "FAIL", f"$150,000 payment accepted - no upper limit. amount={d.get('payment_result',{}).get('amount')}")
elif resp.status_code in (400, 422):
...
print("\n" + "=" * 80)
print("TEST 27: Frontend sends constraints but backend also auto-generates - buffer mismatch")
print("=" * 80)
...
resp = requests.post(FLOW_URL, json={
    "items": [{"description": "Buffer test", "amount": 100.00, "category": "OTHER"}],
...
if resp.status_code == 200:
    ...
    log("T27-buffer-mismatch", "FAIL", "Frontend uses 5% buffer, backend auto uses 10% - inconsistent UX")
else:
