...
results.append(('LIST expenses (empty org)', code, f'{len(resp) if isinstance(resp,list) else resp}'))

# 2. CREATE valid expense
code, resp = api('POST', '/api/v1/expenses', {'amount': 42.50, 'description': 'QA test expense', 'category': 'OFFICE_SUPPLIES', 'vendor': 'Test Vendor'})
results.append(('CREATE expense (valid)', code, f'id={resp.get("id","?")[:8]}... status={resp.get("status")}'))
exp_id1 = resp.get('id', '')

# 3. CREATE TRAVEL
code, resp = api('POST', '/api/v1/expenses', {'amount': 250, 'description': 'Flight to NYC', 'category': 'TRAVEL', 'vendor': 'Delta Airlines'})
results.append(('CREATE TRAVEL expense', code, f'id={resp.get("id","?")[:8]}... status={resp.get("status")}'))
exp_travel = resp.get('id', '')

# 4. CREATE MEALS
code, resp = api('POST', '/api/v1/expenses', {'amount': 15.99, 'description': 'Team lunch', 'category': 'MEALS', 'vendor': 'Restaurant'})
results.append(('CREATE MEALS expense', code, f'status={resp.get("status")}'))
exp_meals = resp.get('id', '')

# 5. CREATE SOFTWARE
code, resp = api('POST', '/api/v1/expenses', {'amount': 99.99, 'description': 'IDE license', 'category': 'SOFTWARE', 'vendor': 'JetBrains'})
results.append(('CREATE SOFTWARE expense', code, f'status={resp.get("status")}'))
exp_sw = resp.get('id', '')

# 6. Negative amount
code, resp = api('POST', '/api/v1/expenses', {'amount': -10, 'description': 'Neg', 'category': 'OTHER', 'vendor': 'X'})
results.append(('CREATE negative amount', code, str(resp)[:120]))

# 7. Zero amount
code, resp = api('POST', '/api/v1/expenses', {'amount': 0, 'description': 'Zero', 'category': 'OTHER', 'vendor': 'X'})
results.append(('CREATE zero amount', code, str(resp)[:120]))

# 8. Amount > 1M
code, resp = api('POST', '/api/v1/expenses', {'amount': 1000001, 'description': 'Huge', 'category': 'TRAVEL', 'vendor': 'X'})
results.append(('CREATE amount > 1M', code, str(resp)[:120]))

# 9. Invalid category
code, resp = api('POST', '/api/v1/expenses', {'amount': 10, 'description': 'Bad', 'category': 'INVALID', 'vendor': 'X'})
...
# 11. XSS
code, resp = api('POST', '/api/v1/expenses', {'amount': 5, 'description': '<script>alert(1)</script>', 'category': 'OTHER', 'vendor': 'X'})
xss_sanitized = 'script' not in resp.get('description', 'script')
results.append(('CREATE XSS in description', code, f'sanitized={xss_sanitized}, desc={resp.get("description","")}'))
exp_xss = resp.get('id', '')
...
# 14. UPDATE expense
code, resp = api('PUT', f'/api/v1/expenses/{exp_id1}', {'amount': 50, 'description': 'Updated', 'category': 'TRAVEL', 'vendor': 'Updated Vendor'})
results.append(('UPDATE expense', code, f'amount={resp.get("amount")}, cat={resp.get("category")}'))

# 15. UPDATE non-existent
code, resp = api('PUT', '/api/v1/expenses/00000000-0000-0000-0000-000000000000', {'amount': 50, 'description': 'Ghost', 'category': 'TRAVEL', 'vendor': 'Ghost'})
...
code, resp = api('PUT', f'/api/v1/expenses/{exp_sw}/reject', {'reason': 'Budget exceeded'})
...
code_c, resp_c = api('POST', '/api/v1/expenses', {'amount': 500, 'description': 'No reason', 'category': 'SOFTWARE', 'vendor': 'X'})
if code_c == 201:
    nr_id = resp_c['id']
...
code, resp = api('PUT', f'/api/v1/expenses/{exp_id1}/request-receipt', {'message': 'Please upload receipt'})
...
# 25. LIST all
code, resp = api('GET', '/api/v1/expenses')
results.append(('LIST all expenses', code, f'count={len(resp) if isinstance(resp,list) else "N/A"}'))

# 26. Report
code, resp = api('GET', '/api/v1/expenses/report')
results.append(('GET report', code, f'total={resp.get("total_expenses")}, amt={resp.get("total_amount")}'))
...
# 29. Wrong org
headers_wrongorg = {'Authorization': f'Bearer {TOKEN}', 'X-Organization-Id': '00000000-0000-0000-0000-000000000000', 'Content-Type': 'application/json'}
...
for name, code, detail in results:
    if code < 300:
        status = 'PASS'
    elif code in (403, 404, 422) and ('non-existent' in name.lower() or 'negative' in name.lower() or 'zero' in name.lower() or 'missing' in name.lower() or 'invalid' in name.lower() or '> 1m' in name.lower() or 'already' in name.lower() or 'without auth' in name.lower() or 'without org' in name.lower() or 'wrong org' in name.lower() or 'delete' in name.lower()):
        status = 'EXPECTED'
    elif code >= 400:
        status = 'FAIL'
    else:
