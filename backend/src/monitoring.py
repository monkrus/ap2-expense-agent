...
class HealthCheck:
    ...
    async def check_database(db_session) -> dict:
        ...
        try:
            ...
            return {"status": "healthy", "response_time_ms": 0}  # Would need timing
        except Exception as e:
...
async def health_check_endpoint(db_session) -> dict:
    ...
    for check_name, check_result in checks["checks"].items():
        if check_result.get("status") == "unhealthy":
            ...
            check_result.get("status") == "warning" and checks["status"] != "unhealthy"
        ):
...
class AlertManager:
    ...
    def send_alert(severity: str, title: str, message: str, details: dict = None):
                        ...
                        "attachments": [
                            {
                                "text": message,
                                "color": (
                                    "danger" if severity == "critical" else "warning"
                                ),
                            }
                        ],
                    },
        ...
        # Log alert
        logger.error(f"ALERT: {title} - {message}", extra=alert_data)
