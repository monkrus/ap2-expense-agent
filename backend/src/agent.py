...
class ExpenseManagementAgent:
    ...
    def get_expense_report(self, user_id: Optional[str] = None) -> Dict:
        ...
        approved_count = len([e for e in filtered_expenses if e['status'] == 'approved'])
    ...
    def get_audit_trail(self, transaction_id: str) -> Dict:
        ...
        payment_mandate = next(
            (m for m in self.mandates['payment'] if m.id == transaction_id),
            None
        )
        ...
        cart_mandate = next(
            (m for m in self.mandates['cart'] if m.id == payment_mandate.cart_mandate_ref),
            None
        )
        
        intent_mandate = next(
            (m for m in self.mandates['intent'] if m.id == cart_mandate.intent_mandate_ref),
            None
