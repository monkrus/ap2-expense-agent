...
results.append(
    (
        ...
        code,
        f"count={len(resp) if isinstance(resp,list) else resp}",
    )
)
...
code, resp = api(
    "GET", "/api/v1/approval-policies/00000000-0000-0000-0000-000000000000"
)
...
code, resp = api(
    "POST", "/api/v1/approval-policies", {"name": "", "conditions": "invalid"}
)
...
code, resp = api(
    ...
    {"amount": 50, "category": "OFFICE_SUPPLIES", "vendor": "Test"},
)
...
if policy_id:
    code, resp = api(
        "POST",
        f"/api/v1/approval-policies/{policy_id}/test",
        {"amount": 50, "category": "OFFICE_SUPPLIES", "vendor": "Test"},
    )
...
code, resp = api(
    "POST",
    "/api/v1/approval-policies/test",
    {"amount": 5000, "category": "TRAVEL", "vendor": "Test"},
)
...
code, resp = api(
    ...
    {
        "amount": 75,
        "description": "Pending for approval test",
        "category": "TRAVEL",
        "vendor": "Test",
    },
)
...
results.append(
    (
        "EXPORT expenses (PDF)",
        code,
        "Got PDF" if isinstance(resp, str) and "PDF" in str(resp) else str(resp)[:80],
    )
)
