...
def main():
    ...
    test_resp = requests.post(f'{BASE_URL}/api/v1/approval-policies/test',
        headers=headers,
    ...
    if test_resp.status_code == 200:
        ...
        if r.get('would_auto_approve'):
            print(f'   [PASS] Would auto-approve via: {policy.get("name", "Unknown") if policy else "N/A"}')
            passed += 1
    ...
    test_resp2 = requests.post(f'{BASE_URL}/api/v1/approval-policies/test',
        headers=headers,
    ...
    mandate_resp = requests.post(f'{BASE_URL}/api/ap2/intent-mandate',
        headers=headers,
        json={
            'constraints': {
                'merchant': 'Amazon',
    ...
    if mandate_resp.status_code in [200, 201]:
        ...
        print(f'   [PASS] Mandate created: {str(mandate_id)[:20] if mandate_id else "N/A"}')
    ...
    expense_resp = requests.post(f'{BASE_URL}/api/v1/expenses',
        headers=headers,
        json={
            'amount': 75.00,
            'vendor': 'Amazon',
    ...
    if expense_resp.status_code in [200, 201]:
        ...
        if auto_approved:
            ...
            print(f'   [PARTIAL] Created but not auto-approved. Status: {expense.get("status")}')
            passed += 0.5
            failed += 0.5
    else:
    ...
    expense_resp2 = requests.post(f'{BASE_URL}/api/v1/expenses',
        headers=headers,
        json={
            'amount': 100.00,
            'vendor': 'BestBuy',
    ...
    if expense_resp2.status_code in [200, 201]:
        ...
        if not auto_approved:
            print(f'   [PASS] Correctly requires manual approval. Status: {expense2.get("status")}')
            passed += 1
        else:
    ...
    stats_resp = requests.get(f'{BASE_URL}/api/v1/approval-policies/analytics/statistics', headers=headers)
    if stats_resp.status_code == 200:
        stats = stats_resp.json()
        print(f'   [PASS] Statistics retrieved')
        print(f'   - Total expenses: {stats.get("total_expenses", 0)}')
        print(f'   - Auto-approved: {stats.get("auto_approved_count", 0)} ({stats.get("auto_approval_rate_percent", 0):.1f}%)')
    ...
    if mandates_resp.status_code == 200:
        ...
        for m in mandates[:3]:
            print(f'   - {str(m.get("id", "?"))[:12]}... | {m.get("status")} | type: {m.get("type", "intent")}')
        passed += 1
    else:
    ...
    print('\n[TEST 8] List approval policies')
    policies_resp = requests.get(f'{BASE_URL}/api/v1/approval-policies', headers=headers)
    if policies_resp.status_code == 200:
        policies = policies_resp.json()
        print(f'   [PASS] Found {len(policies)} policies')
        for p in policies[:3]:
            print(f'   - {p.get("name")} | active: {p.get("is_active")} | auto_approve: {p.get("auto_approve")}')
        passed += 1
    else:
