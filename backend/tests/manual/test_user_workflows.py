...
def test_user_registration_login():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_test(
                "User Login", "FAIL", f"Status: {response.status_code}, {response.text}"
            )
            return None
        ...
        if response.status_code == 200:
            ...
            log_test(
                "Get Current User", "PASS", f"Username: {user_info.get('username')}"
            )
        else:
...
def test_organization_creation(user_context):
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE}/organizations", json=org_data, headers=headers
        )
        ...
        response = requests.post(
            f"{API_BASE}/organizations", json=org_data2, headers=headers
        )
        ...
        if response.status_code == 402:
            ...
            if (
                "1 organization" in friendly_msg
                and "1 organizations" not in friendly_msg
            ):
                log_test(
                    "Tier Limit Enforcement + Grammar Fix",
                    "PASS",
                    f"Correct message: {friendly_msg}",
                )
            else:
                log_test(
                    "Tier Limit Enforcement",
                    "PASS",
                    f"Got 402, but grammar might be off: {friendly_msg}",
                )
...
def test_expense_submission(user_context, org_context):
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE}/expenses", json=expense_data, headers=headers
        )
        ...
        if expense_id:
            response = requests.get(
                f"{API_BASE}/expenses/{expense_id}", headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                log_test(
                    "Get Expense Details",
                    "PASS",
                    f"Status: {expense_details.get('status')}",
                )
            else:
                log_test(
                    "Get Expense Details", "FAIL", f"Status: {response.status_code}"
                )
        else:
            log_test(
                "Get Expense Details", "WARN", "Skipped - no expense ID from creation"
            )
...
def test_auth_error_handling():
    ...
    try:
        ...
        if response.status_code in [401, 403]:
            log_test(
                "No Auth Handling", "PASS", f"Got {response.status_code} as expected"
            )
        else:
            log_test(
                "No Auth Handling",
...
def test_organization_management(user_context, org_context):
    ...
    try:
        ...
        response = requests.get(
            f"{API_BASE}/organizations/{org_context['org_id']}/members", headers=headers
        )
        ...
        if response.status_code == 201:
            ...
            log_test(
                "Send Invitation - Tier Limit",
                "PASS",
                "Hit tier limit (expected for Free tier)",
            )
        else:
            log_test(
                "Send Invitation",
        ...
        if response.status_code == 200:
            ...
            log_test(
                "List Invitations", "PASS", f"Found {len(invitations)} invitation(s)"
            )
        else:
...
def test_multi_tenancy_isolation(user_context, org_context):
    ...
    try:
        ...
        if response.status_code == 403:
            ...
            log_test(
                "Cross-Org Access Block", "WARN", f"Status: {response.status_code}"
            )
        ...
        if response.status_code == 200:
            ...
            if org_context["org_id"] in user_org_ids:
                log_test(
                    "Organization List Isolation", "PASS", "User only sees their orgs"
                )
            else:
                log_test(
                    "Organization List Isolation",
                    "FAIL",
                    "Organization missing from list",
                )
        else:
            log_test(
                "Organization List Isolation", "FAIL", f"Status: {response.status_code}"
            )
