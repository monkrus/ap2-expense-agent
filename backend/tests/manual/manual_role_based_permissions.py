...
class RoleBasedTestRunner:
    ...
    def add_result(
        ...
        """Add test result"""
        result = TestResult(
            test_name, role, passed, message, response_code, expected_code
        )
    ...
    def setup_users(self) -> bool:
        ...
        for username, password in TEST_USERS.items():
            ...
            self.log(
                f"OK {username} logged in (Role: {user.role}, ID: {user.user_id})",
                "SUCCESS",
            )
    ...
    def setup_organization(self) -> bool:
        ...
        self.log(
            f"OK Using existing organization: {org.get('name')} (ID: {self.org_id})",
            "SUCCESS",
        )
        ...
        for username, org_role in role_mapping.items():
            ...
            # Invite user
            invite_data = {"email": user.email, "role": org_role}

            self.log(f"Inviting {username} as {org_role}...")
            response = self.client.post(
                f"/organizations/{self.org_id}/invitations", admin.token, invite_data
            )
            ...
            if response.status_code in [200, 201]:
                ...
                self.log(
                    f"Warning: Could not invite {username}: {response.status_code}",
                    "WARNING",
                )
    ...
    def test_admin_permissions(self):
        ...
        if response.status_code in [200, 201]:
            ...
            self.add_result(
                ...
                "ADMIN",
                True,
                f"Created expense ${expense_data['amount']}",
        ...
        if self.test_expense_ids.get("admintest"):
            ...
            if response.status_code in [200, 201]:
                ...
                self.add_result(
                    ...
                    "ADMIN",
                    True,
                    "Correctly prevented self-approval",
                    response.status_code,
                    403,
                )
    ...
    def test_manager_permissions(self):
        ...
        response = self.client.post(
            "/expenses", manager.token, expense_data, self.org_id
        )
        if response.status_code in [200, 201]:
            expense = response.json()
        ...
        if employee:
            ...
            create_response = self.client.post(
                "/expenses", employee.token, emp_expense_data, self.org_id
            )
            ...
            if create_response.status_code in [200, 201]:
                ...
                response = self.client.put(
                    f"/expenses/{emp_expense_id}/approve",
                    manager.token,
                    {},
                    self.org_id,
                )
                ...
                self.add_result(
                    ...
                    "MANAGER",
                    False,
                    "Could not create employee expense for test",
                    None,
                    None,
                )
        ...
        if self.test_expense_ids.get("testuser"):
            ...
            if response.status_code >= 400:
                ...
                self.add_result(
                    ...
                    "MANAGER",
                    False,
                    "WARNING: Manager approved own expense",
                    response.status_code,
                    403,
                )
        ...
        if self.org_id:
            response = self.client.delete(
                f"/organizations/{self.org_id}", manager.token
            )
            ...
            self.add_result(
                ...
                passed,
                (
                    "Correctly prevented organization deletion"
                    if passed
                    else "WARNING: Deleted org"
                ),
                response.status_code,
                403,
            )
    ...
    def test_accountant_permissions(self):
        ...
        response = self.client.post(
            "/expenses", accountant.token, expense_data, self.org_id
        )
        if response.status_code in [200, 201]:
            expense = response.json()
        ...
        self.add_result(
            ...
            "ACCOUNTANT",
            passed,
            f"Retrieved {count} expenses for review",
            response.status_code,
            200,
        )
        ...
        self.add_result(
            ...
            "ACCOUNTANT",
            True,
            f"Report access status: {response.status_code}",
            response.status_code,
            None,
        )
        ...
        if self.test_expense_ids.get("emptest"):
            ...
            response = self.client.put(
                f"/expenses/{expense_id}/approve", accountant.token, {}, self.org_id
            )
            ...
            if response.status_code >= 400:
                self.add_result(
                    ...
                    "ACCOUNTANT",
                    True,
                    "Correctly restricted from approving",
                    response.status_code,
                    403,
                )
            else:
        ...
        self.add_result(
            ...
            "ACCOUNTANT",
            True,
            f"Export capability status: {response.status_code}",
            response.status_code,
            None,
        )
        ...
        for username in ["emptest", "emptest2"]:
            ...
            if self.test_expense_ids.get(other_user):
                ...
                response = self.client.get(
                    f"/expenses/{other_expense_id}", employee.token, self.org_id
                )
                passed = response.status_code >= 400  # Should fail
                self.add_result(
                    f"View Other's Expense ({username})",
                    "EMPLOYEE",
                    passed,
                    (
                        "Correctly prevented viewing other's expense"
                        if passed
                        else "WARNING: Saw other's expense"
                    ),
                    response.status_code,
                    403,
                )
            ...
            if self.test_expense_ids.get("admintest"):
                ...
                response = self.client.put(
                    f"/expenses/{admin_expense_id}/approve",
                    employee.token,
                    {},
                    self.org_id,
                )
                passed = response.status_code >= 400  # Should fail
                self.add_result(
                    f"Approve Expense ({username})",
                    "EMPLOYEE",
                    passed,
                    (
                        "Correctly prevented approval"
                        if passed
                        else "WARNING: Approved expense"
                    ),
                    response.status_code,
                    403,
                )
            ...
            if self.test_expense_ids.get("admintest"):
                admin_expense_id = self.test_expense_ids["admintest"]
                response = self.client.delete(
                    f"/expenses/{admin_expense_id}", employee.token, self.org_id
                )
                passed = response.status_code >= 400  # Should fail
                self.add_result(
                    f"Delete Other's Expense ({username})",
                    "EMPLOYEE",
                    passed,
                    (
                        "Correctly prevented deletion"
                        if passed
                        else "WARNING: Deleted other's expense"
                    ),
                    response.status_code,
                    403,
                )
            ...
            if self.test_expense_ids.get(username):
                ...
                update_data = {"amount": 50.00, "description": "Updated description"}
                response = self.client.put(
                    f"/expenses/{expense_id}", employee.token, update_data, self.org_id
                )
                passed = response.status_code in [200, 201]
                self.add_result(
                    f"Update Own Expense ({username})",
                    "EMPLOYEE",
                    passed,
    ...
    def test_edge_cases_and_misuse(self):
        ...
        if employee:
            ...
            response = self.client.post(
                "/expenses", employee.token, invalid_expense, self.org_id
            )
            ...
            self.add_result(
                ...
                (
                    ...
                    if passed
                    else "WARNING: Accepted negative amount"
                ),
                response.status_code,
                400,
            )
        ...
        if employee:
            ...
            response = self.client.post(
                "/expenses", employee.token, huge_expense, self.org_id
            )
            ...
            self.add_result(
                ...
                "EDGE_CASE",
                True,
                f"Large expense handling: {response.status_code}",
                response.status_code,
                None,
            )
        ...
        if employee:
            ...
            response = self.client.post(
                "/expenses", employee.token, sql_injection_expense, self.org_id
            )
        ...
        if employee:
            ...
            response = self.client.post(
                "/expenses", employee.token, xss_expense, self.org_id
            )
            passed = response.status_code in [200, 201] or response.status_code == 400
            self.add_result(
                "XSS Protection",
        ...
        if employee:
            ...
            response = self.client.post(
                "/expenses", employee.token, invalid_date_expense, self.org_id
            )
            passed = response.status_code >= 400  # Should reject
            self.add_result(
                "Invalid Date Format",
                ...
                (
                    ...
                    if passed
                    else "WARNING: Accepted invalid date"
                ),
                response.status_code,
                400,
            )
        ...
        if admin and manager:
            ...
            self.add_result(
                ...
                passed,
                (
                    "Correctly prevented unauthorized org access"
                    if passed
                    else "WARNING: Accessed wrong org"
                ),
                response.status_code,
                403,
            )
        ...
        self.add_result(
            ...
            (
                ...
                if passed
                else "WARNING: Invalid token accepted"
            ),
            response.status_code,
            401,
        )
        ...
        if employee:
            ...
            response = self.client.post(
                "/expenses", employee.token, incomplete_expense, self.org_id
            )
            passed = response.status_code >= 400  # Should reject
            self.add_result(
                ...
                (
                    ...
                    if passed
                    else "WARNING: Accepted incomplete data"
                ),
                response.status_code,
                400,
            )
        ...
        if employee:
            ...
            response1 = self.client.post(
                "/expenses", employee.token, duplicate_expense, self.org_id
            )
            time.sleep(0.5)
        ...
        if employee:
            ...
            for i in range(10):
                ...
                response = self.client.post(
                    "/expenses", employee.token, rapid_expense, self.org_id
                )
            ...
            self.add_result(
                ...
                (
                    ...
                    if rate_limit_hit
                    else "No rate limiting detected (consider implementing)"
                ),
                429 if rate_limit_hit else 200,
                None,
            )
    ...
    def print_summary(self):
        ...
        for role, stats in sorted(by_role.items()):
            ...
            role_pass_rate = (
                (stats["passed"] / role_total * 100) if role_total > 0 else 0
            )
            print(
                f"{role:<20} {stats['passed']:<10} {stats['failed']:<10} {role_total:<10} {role_pass_rate:.1f}%"
            )
            ...
            for result in self.results:
                if not result.passed:
                    ...
                    if result.response_code and result.expected_code:
                        print(
                            f"    Expected: {result.expected_code}, Got: {result.response_code}"
                        )
        ...
        if pass_rate >= 90:
            ...
            self.log(
                f"WARNING FAIR: {pass_rate:.1f}% pass rate (improvements needed)",
                "WARNING",
            )
        else:
            self.log(
                f"ERROR POOR: {pass_rate:.1f}% pass rate (critical issues found)",
                "ERROR",
            )
