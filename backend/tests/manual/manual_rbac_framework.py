...
class Logger:
    ...
    def test_pass(test_name: str, role: str, message: str, category: str):
        print(
            f"{Color.GREEN}[PASS]{Color.ENDC} | {category:12} | {role:12} | {test_name}: {message}",
            flush=True,
        )
    ...
    def test_fail(test_name: str, role: str, message: str, category: str):
        print(
            f"{Color.RED}[FAIL]{Color.ENDC} | {category:12} | {role:12} | {test_name}: {message}",
            flush=True,
        )
...
class RBACTestFramework:
            ...
            for attempt in range(max_retries):
                ...
                if "rate_limit" in str(data.get("error", "")).lower():
                    if attempt < max_retries - 1:
                        ...
                        Logger.error(
                            f"Failed to login {user_info['username']} after {max_retries} attempts: rate limited"
                        )
                        return False
                else:
                    Logger.error(
                        f"Failed to login {user_info['username']}: {data.get('error')}"
                    )
                    return False
    ...
    def add_result(self, result: TestResult):
        ...
        if result.passed:
            Logger.test_pass(
                result.test_name, result.role, result.message, result.category.value
            )
        else:
            Logger.test_fail(
                result.test_name, result.role, result.message, result.category.value
    ...
    def generate_report(self, duration: float):
        ...
        print(
            f"Pass Rate:       {Color.GREEN if pass_rate >= 90 else Color.YELLOW}{pass_rate:.1f}%{Color.ENDC}"
        )
        ...
        for category, stats in sorted(by_category.items()):
            ...
            print(
                f"{category:<20} {stats['passed']:<10} {stats['failed']:<10} {cat_total:<10} {cat_pass_rate:>6.1f}%"
            )
        ...
        for role, stats in sorted(by_role.items()):
            ...
            print(
                f"{role:<20} {stats['passed']:<10} {stats['failed']:<10} {role_total:<10} {role_pass_rate:>6.1f}%"
            )
        ...
        if pass_rate >= 95:
            ...
            Logger.success(
                f"GOOD: {pass_rate:.1f}% pass rate - Minor issues to address"
            )
        elif pass_rate >= 70:
...
class EmployeeTests:
    ...
    def test_typical_workflows(self):
        ...
        if expense_id:
            ...
            response = self.client.put(
                f"/expenses/{expense_id}", self.user.token, update_data
            )
            duration = (time.time() - start) * 1000
            ...
            self.fw.add_result(
                TestResult(
                    ...
                    passed=passed,
                    message=(
                        "Updated expense description" if passed else "Failed to update"
                    ),
                    expected_status=200,
                    actual_status=response.status_code,
                    duration_ms=duration,
    ...
    def test_permission_boundaries(self):
        ...
        if expense_id:
            response = self.client.put(
                f"/expenses/{expense_id}/approve", self.user.token, {}
            )
            ...
            self.fw.add_result(
                TestResult(
                    ...
                    passed=passed,
                    message=(
                        "Correctly denied approval permission"
                        if passed
                        else "WARNING: Employee approved expense"
                    ),
                    expected_status=403,
                    actual_status=response.status_code,
                )
...
class ManagerTests:
    ...
    def test_approval_workflows(self):
        ...
        if employee_expense_id:
            response = self.client.put(
                f"/expenses/{employee_expense_id}/approve", self.user.token, {}
            )
            ...
            self.fw.add_result(
                TestResult(
                    ...
                    message=(
                        ...
                        if passed
                        else f"Approval status: {response.status_code}"
                    ),
                    expected_status=200,
                    actual_status=response.status_code,
                )
        ...
        if manager_expense_id:
            response = self.client.put(
                f"/expenses/{manager_expense_id}/approve", self.user.token, {}
            )
            ...
            self.fw.add_result(
                TestResult(
                    ...
                    message=(
                        ...
                        if passed
                        else "WARNING: Manager approved own expense"
                    ),
                    expected_status=403,
                    actual_status=response.status_code,
                )
    ...
    def test_permission_boundaries(self):
        ...
        self.fw.add_result(
            TestResult(
                ...
                passed=passed,
                message=(
                    "Correctly prevented org deletion"
                    if passed
                    else "WARNING: Manager deleted org"
                ),
                expected_status=403,
                actual_status=response.status_code,
            )
...
class AccountantTests:
    ...
    def test_typical_workflows(self):
        ...
        response = self.client.get(
            "/expenses/export", self.user.token, params={"format": "csv"}
        )
    ...
    def test_audit_workflows(self):
        ...
        if expense_id:
            response = self.client.put(
                f"/expenses/{expense_id}/request-receipt", self.user.token
            )
        ...
        if expense_id:
            ...
            self.fw.add_result(
                TestResult(
                    ...
                    passed=passed,
                    message=(
                        "Correctly protected audit trail"
                        if passed
...
class AdminTests:
    ...
    def test_typical_workflows(self):
        ...
        invite_data = {"email": "newmember@test.com", "role": "member"}
        response = self.client.post(
            f"/organizations/{self.fw.org_id}/invitations", self.user.token, invite_data
        )
        ...
        self.fw.add_result(
            TestResult(
                ...
                message=(
                    "Invited new member"
                    if passed
                    else f"Invite status: {response.status_code}"
                ),
                expected_status=201,
                actual_status=response.status_code,
            )
        ...
        self.fw.add_result(
            TestResult(
                ...
                message=(
                    ...
                    if passed
                    else f"Creation status: {response.status_code}"
                ),
                expected_status=201,
                actual_status=response.status_code,
            )
        ...
        if response.status_code in [200, 201]:
            ...
            self.fw.add_result(
                TestResult(
                    ...
                    passed=passed,
                    message=(
                        "Employee submitted, Manager approved"
                        if passed
                        else "Approval chain incomplete"
                    ),
                    actual_status=response.status_code,
                )
            )
            ...
            self.fw.add_result(
                TestResult(
                    ...
                    passed=passed,
                    message=(
                        "Accountant audited approved expense"
                        if passed
                        else "Audit access denied"
                    ),
                    actual_status=response.status_code,
                )
            )
