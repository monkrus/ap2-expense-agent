...
class TestResults:
    ...
    def add_test(self, name: str, passed: bool, severity: str = "normal", message: str = ""):
        self.total += 1
        test_result = {
            "name": name,
            "passed": passed,
...
class TestContext:
    ...
    def register_user(self, username: str, password: str, email: str, full_name: str, role: str = "employee") -> bool:
        """Register a new user"""
        ...
        try:
            ...
            if response.status_code == 201:
                ...
                TestLogger.error(f"Registration failed: {response.status_code} - {response.text[:200]}")
                return False
    ...
    def login(self, username: str, password: str, role_key: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                TestLogger.error(f"Login failed: {response.status_code} - {response.text[:100]}")
                return False
        except Exception as e:
...
class ManagerWorkflowTests:
    ...
    def run(self):
        ...
        TestLogger.section("Testing: View Team Expenses, Approve, Reject, Filter, Export")
...
class AdminWorkflowTests:
    ...
    def run(self):
        ...
        TestLogger.section("Testing: Manage Organization, Members, Settings (NOT Owner)")
        ...
        response = requests.patch(
            ...
            headers=self.ctx.headers("admin"),
            json={
                "description": "Updated by admin for testing"
            }
        )
        ...
        admin_member = next((m for m in members if m["user_id"] == self.ctx.users["admin"]["id"]), None)
        ...
        member_to_update = next((m for m in members if m.get("role") == "member" and m["user_id"] != admin_user_id), None)
...
class SecurityTests:
    ...
    def run(self):
        ...
        TestLogger.section("Testing: Multi-Tenancy, Permission Boundaries, Data Isolation")
...
class EndToEndIntegrationTests:
    ...
    def run(self):
        ...
        TestLogger.section("Complete Business Cycle: Submit -> Review -> Approve -> Export")
        ...
        if response.status_code == 200:
            ...
            integration_expenses = [e for e in all_expenses if "Integration test" in e.get("description", "")]
...
def run_complete_role_workflow_validation():
    ...
    users_to_create = [
        ("employee", f"emp_workflow_{timestamp}", "EmpFlow123!", f"emp_{timestamp}@test.com", "Employee Tester"),
        ("manager", f"mgr_workflow_{timestamp}", "MgrFlow123!", f"mgr_{timestamp}@test.com", "Manager Tester"),
        ("admin", f"adm_workflow_{timestamp}", "AdmFlow123!", f"adm_{timestamp}@test.com", "Admin Tester"),
        ("owner", f"own_workflow_{timestamp}", "OwnFlow123!", f"own_{timestamp}@test.com", "Owner Tester"),
        ("accountant", f"acc_workflow_{timestamp}", "AccFlow123!", f"acc_{timestamp}@test.com", "Accountant Tester"),
    ]
    ...
    for workflow_name, results in all_results:
        ...
        print(f"{workflow_name:<20} {results.total:<8} {color}{results.passed:<8}{RESET} {RED}{results.failed:<8}{RESET} {BOLD}{RED}{results.critical_failed:<10}{RESET} {color}{pass_rate:>6.1f}%{RESET}")

    print("=" * 80)
    overall_pass_rate = (total_passed / total_tests * 100) if total_tests > 0 else 0
    overall_color = GREEN if overall_pass_rate >= 90 else YELLOW if overall_pass_rate >= 75 else RED

    print(f"{BOLD}{'TOTAL':<20} {total_tests:<8} {overall_color}{total_passed:<8}{RESET} {BOLD}{RED}{total_failed:<8}{RESET} {BOLD}{RED}{total_critical:<10}{RESET} {BOLD}{overall_color}{overall_pass_rate:>6.1f}%{RESET}")

    # Critical issues summary
    if total_critical > 0:
        TestLogger.critical(f"\n{total_critical} CRITICAL ISSUES FOUND - REQUIRES IMMEDIATE ATTENTION")
        for workflow_name, results in all_results:
    ...
    if overall_pass_rate >= 90 and total_critical == 0:
        TestLogger.success(f"{BOLD}WORKFLOW STATUS: PASSING{RESET} - All workflows functional, system ready for production")
    elif overall_pass_rate >= 75:
        TestLogger.warning(f"{BOLD}WORKFLOW STATUS: ISSUES FOUND{RESET} - Most workflows functional, minor fixes needed")
    else:
        TestLogger.critical(f"{BOLD}WORKFLOW STATUS: CRITICAL FAILURES{RESET} - Major workflow issues, not production-ready")
