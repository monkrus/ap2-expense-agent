...
class UserBehavior(SequentialTaskSet):
    ...
    def on_start(self):
        ...
        response = self.client.post(
            "/api/v1/auth/register", json=register_payload, name="/api/v1/auth/register"
        )
        ...
        if response.status_code == 201:
            ...
            response = self.client.post(
                "/api/v1/auth/login", json=login_payload, name="/api/v1/auth/login"
            )
    ...
    def list_expenses(self):
        ...
        params = {"limit": random.choice([10, 20, 50]), "offset": 0}
    ...
    def on_start(self):
        ...
        if response.status_code == 201:
            ...
            if response.status_code == 200:
                ...
                response = self.client.post(
                    "/api/v1/organizations", json=org_payload, headers=self.headers
                )
                if response.status_code == 201:
                    self.organization_id = response.json()["id"]
                    self.headers["X-Organization-Id"] = self.organization_id
